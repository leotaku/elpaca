;;; elpaca-use-package.el --- use-package support for Elpaca  -*- lexical-binding: t; -*-
...
(defvar elpaca-use-package--keyword :elpaca)
...
see `use-package' docs for STATE."
  ...
  (let* ((rest (cl-loop for i below (length rest)
         ...
         (arg (car args)))
    `((elpaca ,@(cond ((eq t arg) (list name))
                      (t args))
        ,@body))))
...
(defun elpaca-use-package--setup ()
  "Setup elpaca's `use-package'."
  (unless (memq elpaca-use-package--keyword use-package-keywords)
    (setq use-package-keywords
          (let ((tail (memq :no-require use-package-keywords)))
            (append
             ...
             (list elpaca-use-package--keyword)
             tail)))
    ...
    (add-to-list 'use-package-defaults
                 (list elpaca-use-package--keyword '(list t)
...
(defun elpaca-use-package--teardown ()
  ...
  (setq use-package-keywords elpaca-use-package--original-keywords)
  (setf (alist-get :elpaca use-package-defaults nil 'remove) nil)
...
(define-minor-mode elpaca-use-package-mode
  ...
  (if elpaca-use-package-mode
      (elpaca-use-package--setup)
    (elpaca-use-package--teardown)))

(provide 'elpaca-use-package)
;;; elpaca-use-package.el ends here

;; @FIX: We should be able to byte compile this file, but I don't want Elpaca as
;; a whole to depend on use-package. This should be registered with Elpaca as a
;; separate package. Waiting to do this until I improve the init
;; snippet/bootstrapping situation. ~NV 2022-08-13
